		...
		if started:
			...
			# extracted by the IDF converter
				...
				try:
					...
					for var in ['x', 'y', 'size']:
						scope = {'x':x, 'xi':xi, 'y':y, 'yi':yi, 'size':size, 'sizei':sizei}
						exec("vi = %si" % var, scope)
						exec("v = %s" % var, scope)
						# nothing
						if scope['vi']['L'] == None and scope['vi']['R'] == None:
							val = 'not in IDF'
						# only left eye
						elif scope['vi']['L'] != None and scope['vi']['R'] == None:
							val = float(line[scope['vi']['L']])
						# only right eye
						elif scope['vi']['L'] == None and scope['vi']['R'] != None:
							val = float(line[scope['vi']['R']])
						# average the two eyes, but only if they both
						# contain valid data
						elif scope['vi']['L'] != None and scope['vi']['R'] != None:
							if float(line[scope['vi']['L']]) == 0:
								val = float(line[scope['vi']['R']])
							elif float(line[scope['vi']['R']]) == 0:
								val = float(line[scope['vi']['L']])
							else:
								val = (float(line[scope['vi']['L']]) + float(line[scope['vi']['R']])) / 2.0
						scope['v'].append(val)
					# extract time data
