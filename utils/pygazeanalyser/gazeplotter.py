...
def draw_heatmap(fixations, dispsize, imagefile=None, durationweight=True, alpha=0.5, savefilename=None):
	...
	# matrix of zeroes
	strt = int(gwh/2) # dx: bug seems here, trans to int
	heatmapsize = int(dispsize[1] + 2*strt), int(dispsize[0] + 2*strt) # dx: seems bug here, trans to int
...
def draw_display(dispsize, imagefile=None):
	...
	if imagefile != None:
		...
		x = int(dispsize[0]/2 - w/2) #dx: 这里似乎有 bug, 所以我加了两个 int 转换
